			...
			int mnt_flags, const char *name, void *data)
{
	...
	if (err)
		...
		if (!susfs_is_boot_completed_triggered && !err && susfs_is_auto_add_sus_ksu_default_mount_enabled &&
				(!(mnt_flags & (MS_REMOUNT | MS_BIND | MS_SHARED | MS_PRIVATE | MS_SLAVE | MS_UNBINDABLE)))) {
			if (susfs_is_current_ksu_domain()) {
				susfs_auto_add_sus_ksu_default_mount(path->dentry->d_iname);
			}
		}
#endif
	return err;
